...
unsigned char radar_cmd[][7] = {

    {0xff,0x01,0x00,0x08,0x00,0x3f,0x48},/*上*/
    {0xff,0x01,0x00,0x10,0x00,0x3f,0x50},/*下*/
    {0xff,0x01,0x00,0x04,0x3f,0x00,0x44},/*左*/
    {0xff,0x01,0x00,0x02,0x3f,0x00,0x42},/*右*/
...
int main(void)
{
    ...
    {
       scanf("%d", &select); 
       uart_write(fd,&radar_cmd[select][0], 1); 
       uart_write(fd,&radar_cmd[select][1], 1); 
       uart_write(fd,&radar_cmd[select][2], 1); 
       uart_write(fd,&radar_cmd[select][3], 1); 
       uart_write(fd,&radar_cmd[select][4], 1); 
       uart_write(fd,&radar_cmd[select][5], 1); 
       uart_write(fd,&radar_cmd[select][6], 1); 
    }
